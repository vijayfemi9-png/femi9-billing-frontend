...
var Utils$1 = class Utils {
  ...
  shadeRGBColor(percent, color) {
    const f2 = color.split(","), t2 = percent < 0 ? 0 : 255, p2 = percent < 0 ? percent * -1 : percent, R = parseInt(f2[0].slice(4), 10), G = parseInt(f2[1], 10), B = parseInt(f2[2], 10);
    return "rgb(" + (Math.round((t2 - R) * p2) + R) + "," + (Math.round((t2 - G) * p2) + G) + "," + (Math.round((t2 - B) * p2) + B) + ")";
  }
  shadeHexColor(percent, color) {
    const f2 = parseInt(color.slice(1), 16), t2 = percent < 0 ? 0 : 255, p2 = percent < 0 ? percent * -1 : percent, R = f2 >> 16, G = f2 >> 8 & 255, B = f2 & 255;
  ...
  static preciseAddition(a2, b2) {
    ...
    const bDecimals = (String(b2).split(".")[1] || "").length;
  ...
  static mod(a2, b2, p2 = 7) {
    const big = Math.pow(10, p2 - Math.floor(Math.log10(Math.max(a2, b2))));
...
var DateTime = class {
  ...
  formatDate(date, format) {
    ...
    const y2 = utc ? date.getUTCFullYear() : date.getFullYear();
        ...
        radialBar: {
          ...
          dataLabels: {
            ...
            total: {
              ...
              color: void 0,
              formatter(w) {
                return w.globals.seriesTotals.reduce((a2, b2) => a2 + b2, 0) / w.seriesData.series.length + "%";
              }
            }
          },
          barLabels: {
...
var CoreUtils = class {
  ...
  getStackedSeriesTotalsByGroups() {
    ...
    w.labelData.seriesGroups.forEach((sg) => {
      ...
      const excludedIndices = w.seriesData.series.map((_, fi) => includedIndexes.indexOf(fi) === -1 ? fi : -1).filter((f2) => f2 !== -1);
  ...
  setSeriesYAxisMappings() {
    ...
    const seriesNameArrayStyle = this.w.seriesData.series.length > cnf.yaxis.length || cnf.yaxis.some((a2) => Array.isArray(a2.seriesName));
  ...
  seriesHaveSameValues(index) {
    return this.w.seriesData.series[index].every((val, i2, arr) => val === arr[0]);
  }
  ...
  getCalculatedRatios() {
    ...
    if (w.globals.seriesYAxisReverseMap.length > 0) {
      const scaleBaseLineYScale = (y2, i2) => {
  ...
  getLogYRatios(yRatio) {
    ...
    gl.logYRange = gl.yRange.map((_, i2) => {
      ...
      if (w.config.yaxis[yAxisIndex] && this.w.config.yaxis[yAxisIndex].logarithmic) {
        ...
        gl.yLogRatio[i2] = range / this.w.layout.gridHeight;
        return range;
      }
    });
...
var Graphics = class _Graphics {
  ...
  drawPath({
    ...
    const p2 = w.dom.Paper.path(d).attr({
      fill,
  ...
  curve(x1, y1, x2, y2, x, y3) {
    const curve = ["C", x1, y1, x2, y2, x, y3].join(" ");
    return curve;
  }
  quadraticCurve(x1, y1, x, y2) {
    const curve = ["Q", x1, y1, x, y2].join(" ");
    return curve;
  }
  arc(rx, ry, axisRotation, largeArcFlag, sweepFlag, x, y2, relative = false) {
    let coord = "A";
    if (relative) coord = "a";
    const arc = [coord, rx, ry, axisRotation, largeArcFlag, sweepFlag, x, y2].join(
      " "
    );
    return arc;
  }
  ...
  drawPattern(style, width, height, stroke = "#a8a8a8", strokeWidth = 0) {
    const w = this.w;
    const p2 = w.dom.Paper.pattern(width, height, (add) => {
  ...
  drawGradient(style, gfrom, gto, opacityFrom, opacityTo, size = null, stops = null, colorStops = [], i2 = 0) {
    ...
    if (!colorStops || colorStops.length === 0) {
      ...
      g = w.dom.Paper.gradient(radial ? "radial" : "linear", (add) => {
        const gradientStops = Array.isArray(colorStops[i2]) ? colorStops[i2] : colorStops;
  ...
  pathMouseDown(path, e2) {
    ...
    if (path.node.getAttribute("selected") === "true") {
      ...
      if (w.interact.selectedDataPoints[i2].indexOf(j) > -1) {
  ...
  rotateAroundCenter(el) {
    ...
    const y2 = coord.y + coord.height / 2;
    return {
      x,
      y: y2
    };
    ...
    if (Array.isArray(p2.x)) {
      for (let q = 0; q < p2.x.length; q++) {
        ...
        if (shouldMarkerDraw || alwaysDrawMarker || hasDiscreteMarkers) {
          ...
          if (p2.x[q] < -w.globals.markers.largestSize || p2.x[q] > w.layout.gridWidth + w.globals.markers.largestSize || p2.y[q] < -w.globals.markers.largestSize || p2.y[q] > w.layout.gridHeight + w.globals.markers.largestSize) {
            opts.pSize = 0;
          }
          if (!invalidMarker) {
...
var Scatter = class {
  ...
  draw(elSeries, j, opts) {
    ...
    if (Array.isArray(pointsPos.x)) {
      for (let q = 0; q < pointsPos.x.length; q++) {
        ...
        const y2 = pointsPos.y[q];
        radius = radius || 0;
        if (y2 === null || typeof w.seriesData.series[realIndex][dataPointIndex] === "undefined") {
          shouldDraw = false;
        }
        if (shouldDraw) {
  ...
  drawPoint(x, y2, radius, realIndex, dataPointIndex, j) {
    ...
    const el = graphics.drawMarker(x, y2, markerConfig);
    if (w.config.series[i2].data[dataPointIndex]) {
      if (w.config.series[i2].data[dataPointIndex].fillColor) {
...
var DataLabels = class {
  ...
  dataLabelsCorrection(x, y2, val, i2, dataPointIndex, alwaysDrawDataLabel, fontSize) {
    ...
    if (y2 < 0) y2 = 0;
    if (y2 > w.layout.gridHeight + height) y2 = w.layout.gridHeight + height / 2;
    ...
    const len = w.globals.dataLabelsRects[i2].length - 2;
    ...
    if (typeof w.globals.dataLabelsRects[i2][len] !== "undefined") {
      const lastDataLabelRect = w.globals.dataLabelsRects[i2][lastDrawnIndex];
      if (
...
var AxesUtils = class {
  ...
  translateYAxisIndex(index) {
    ...
    const newStyle = w.seriesData.series.length > yaxis.length || yaxis.some((a2) => Array.isArray(a2.seriesName));
    if (newStyle) {
      return index;
    } else {
  ...
  drawGridBands(xCount, tickAmount) {
    ...
    if (((_b = w.config.grid.column.colors) == null ? void 0 : _b.length) > 0) {
      ...
      for (let i2 = 0, c2 = 0; i2 < xCount; i2++, c2++) {
        ...
        if (w.config.xaxis.type === "datetime") {
          x1 = this.xaxisLabels[i2].position;
          x2 = (((_c = this.xaxisLabels[i2 + 1]) == null ? void 0 : _c.position) || w.layout.gridWidth) - this.xaxisLabels[i2].position;
        }
...
var Scales = class {
  ...
  setYScaleForIndex(index, minY, maxY) {
    const gl = this.w.globals;
    const cnf = this.w.config;
    const y2 = gl.isBarHorizontal ? cnf.xaxis : cnf.yaxis[index];
...
var Range = class {
  ...
  getMinYMaxY(startingSeriesIndex, lowestY = Number.MAX_VALUE, highestY = -Number.MAX_VALUE, endingSeriesIndex = null) {
    ...
    for (let i2 = startingSeriesIndex; i2 < endingSeriesIndex; i2++) {
      ...
      if (this.w.labelData.labels.length && cnf.xaxis.type !== "datetime" && this.w.seriesData.series.reduce((a2, c2) => a2 + c2.length, 0) !== 0) {
      ...
      if (autoScaleYaxis) {
        if (cnf.xaxis.min) {
          for (; firstXIndex < lastXIndex && this.w.seriesData.seriesX[i2][firstXIndex] < cnf.xaxis.min; firstXIndex++) {
          }
        }
        if (cnf.xaxis.max) {
          for (; lastXIndex > firstXIndex && this.w.seriesData.seriesX[i2][lastXIndex] > cnf.xaxis.max; lastXIndex--) {
          }
        }
      }
      for (let j = firstXIndex; j <= lastXIndex && j < this.w.seriesData.series[i2].length; j++) {
        ...
        if (val !== null && Utils$1.isNumber(val)) {
            ...
            case "candlestick":
              {
                if (typeof this.w.candleData.seriesCandleC[i2][j] !== "undefined") {
                  ...
                  lowestY = Math.min(lowestY, this.w.candleData.seriesCandleL[i2][j]);
                }
              }
              break;
            case "boxPlot":
              {
                if (typeof this.w.candleData.seriesCandleC[i2][j] !== "undefined") {
                  ...
                  lowestY = Math.min(lowestY, this.w.candleData.seriesCandleO[i2][j]);
                }
              }
              break;
          }
          ...
          if (this.w.seriesData.seriesGoals[i2] && this.w.seriesData.seriesGoals[i2][j] && Array.isArray(this.w.seriesData.seriesGoals[i2][j])) {
            this.w.seriesData.seriesGoals[i2][j].forEach((g) => {
  ...
  setYRange() {
    ...
    cnf.series.forEach((s2) => {
      const type = s2.type || cnf.chart.type;
      switch (type) {
        case "bar":
        case "column":
  ...
  setXRange() {
    ...
    const getInitialMinXMaxX = () => {
      for (let i2 = 0; i2 < this.w.seriesData.series.length; i2++) {
        if (this.w.labelData.labels[i2]) {
          for (let j = 0; j < this.w.labelData.labels[i2].length; j++) {
            if (this.w.labelData.labels[i2][j] !== null && Utils$1.isNumber(this.w.labelData.labels[i2][j])) {
              gl.maxX = Math.max(gl.maxX, this.w.labelData.labels[i2][j]);
              gl.initialMaxX = Math.max(gl.maxX, this.w.labelData.labels[i2][j]);
              gl.minX = Math.min(gl.minX, this.w.labelData.labels[i2][j]);
              gl.initialMinX = Math.min(gl.minX, this.w.labelData.labels[i2][j]);
            }
          }
        }
      }
    ...
    seriesGroups.forEach((group) => {
      ...
      indicesOfSeriesInGroup.forEach((i2) => {
        ...
        for (let j = 0; j < this.w.seriesData.series[gl.maxValsInArrayIndex].length; j++) {
          ...
          const stackSeries = this.w.config.chart.stacked && !gl.comboCharts || this.w.config.chart.stacked && gl.comboCharts && (!this.w.config.chart.stackOnlyBar || ((_b = (_a = this.w.config.series) == null ? void 0 : _a[i2]) == null ? void 0 : _b.type) === "bar" || ((_d = (_c = this.w.config.series) == null ? void 0 : _c[i2]) == null ? void 0 : _d.type) === "column");
          if (stackSeries) {
            if (this.w.seriesData.series[i2][j] !== null && Utils$1.isNumber(this.w.seriesData.series[i2][j])) {
              this.w.seriesData.series[i2][j] > 0 ? stackedPoss[group][j] += parseFloat(this.w.seriesData.series[i2][j]) + 1e-4 : stackedNegs[group][j] += parseFloat(this.w.seriesData.series[i2][j]);
            }
          }
        }
      });
...
var YAxis = class {
  ...
  drawYaxis(realIndex) {
    ...
    if (w.config.yaxis[realIndex].labels.show) {
      ...
      let firstLabel = null;
      for (let i2 = tickAmount; i2 >= 0; i2--) {
        const val = lbFormatter(labels[i2], i2, w);
  ...
  parseDataAxisCharts(ser) {
    ...
    ser.forEach((s2, i2) => {
      if (s2.name !== void 0) {
        ...
        this.w.seriesData.seriesNames.push("series-" + parseInt(i2 + 1, 10));
      }
    });
    this.coreUtils.setSeriesYAxisMappings();
    const buckets = [];
    const groups = [...new Set(cnf.series.map((s2) => s2.group))];
    cnf.series.forEach((s2, i2) => {
          ...
          if (overwriteInitialConfig) {
            ...
            if (options2.series) {
              for (let i2 = 0; i2 < w.globals.collapsedSeriesIndices.length; i2++) {
                const series = w.config.series[w.globals.collapsedSeriesIndices[i2]];
              ...
              for (let i2 = 0; i2 < w.globals.ancillaryCollapsedSeriesIndices.length; i2++) {
                const series = w.config.series[w.globals.ancillaryCollapsedSeriesIndices[i2]];
...
var Utils2 = class {
  ...
  isXoverlap(j) {
    ...
    const seriesX = w.seriesData.seriesX.filter((s2) => typeof s2[0] !== "undefined");
    if (seriesX.length > 0) {
  ...
  isInitialSeriesSameLen() {
    ...
    for (let i2 = 0; i2 < initialSeries.length - 1; i2++) {
      if (!((_b = initialSeries[i2]) == null ? void 0 : _b.data) || !((_c = initialSeries[i2 + 1]) == null ? void 0 : _c.data)) return true;
...
var Labels = class {
  ...
  printLabels({ i: i2, j, values, ttItems, shared, e: e2 }) {
    ...
    for (let t2 = 0, inverset = w.seriesData.series.length - 1; t2 < w.seriesData.series.length; t2++, inverset--) {
      ...
      if (w.globals.axisCharts) {
        const getValBySeriesIndex = (index) => {
          ...
          if (w.axisFlags.isRangeData) {
            return f2.yLbFormatter((_b = (_a2 = w.rangeData.seriesRangeStart) == null ? void 0 : _a2[index]) == null ? void 0 : _b[j], {
              series: w.rangeData.seriesRangeStart,
              seriesIndex: index,
              dataPointIndex: j,
              w
            }) + " - " + f2.yLbFormatter((_d = (_c = w.rangeData.seriesRangeEnd) == null ? void 0 : _c[index]) == null ? void 0 : _d[j], {
          ...
          return f2.yLbFormatter(w.seriesData.series[index][j], {
            series: w.seriesData.series,
            seriesIndex: index,
            dataPointIndex: j,
            w
        ...
        if (shared) {
          ...
          if (hasGoalValues(i2) && Array.isArray(w.seriesData.seriesGoals[i2][j])) {
            goalVals = w.seriesData.seriesGoals[i2][j].map((goal) => {
              return {
                attrs: goal,
                val: f2.yLbFormatter(goal.value, {
                  seriesIndex: i2,
      ...
      if (j === null) {
        val = f2.yLbFormatter(w.seriesData.series[i2], __spreadProps(__spreadValues({}, w), {
          seriesIndex: i2,
          dataPointIndex: i2
        }));
      }
  ...
  getFormatters(i2) {
    ...
    if (w.formatters.ttVal !== void 0) {
      if (Array.isArray(w.formatters.ttVal)) {
        yLbFormatter = w.formatters.ttVal[i2] && w.formatters.ttVal[i2].formatter;
        yLbTitleFormatter = w.formatters.ttVal[i2] && w.formatters.ttVal[i2].title && w.formatters.ttVal[i2].title.formatter;
      } else {
  ...
  getValuesToPrint({ i: i2, j }) {
    ...
    if (w.seriesData.seriesZ.length > 0 && w.seriesData.seriesZ[i2].length > 0) {
      zVal = zFormatter(w.seriesData.seriesZ[i2][j], w);
    }
    if (typeof w.config.xaxis.tooltip.formatter === "function") {
...
var Position = class {
  ...
  moveTooltip(cx, cy, markerSize = null) {
    ...
    if (w.config.tooltip.followCursor) {
      ...
      y2 = ttCtx.e.clientY + w.layout.translateY - seriesBound.top;
      if (y2 > w.layout.gridHeight / 2) {
        y2 = y2 - ttCtx.tooltipRect.ttHeight;
      }
    } else {
      if (!w.globals.isBarHorizontal) {
        if (tooltipRect.ttHeight / 2 + y2 > w.layout.gridHeight) {
          y2 = w.layout.gridHeight - tooltipRect.ttHeight + w.layout.translateY;
        }
      }
    }
    if (!isNaN(x)) {
  ...
  moveDynamicPointsOnHover(j) {
    ...
    if (points.length) {
      for (let p2 = 0; p2 < w.seriesData.series.length; p2++) {
        const pointArr = pointsArr[p2];
        if (w.globals.comboCharts) {
        ...
        if (pointArr && pointArr.length) {
          ...
          if (w.config.chart.type === "rangeArea" && !w.globals.comboCharts) {
            const rangeStartIndex = j + w.seriesData.series[p2].length;
...
var Marker = class {
  ...
  enlargePoints(j) {
    ...
    for (let p2 = 0; p2 < points.length; p2++) {
      const rel = points[p2].getAttribute("rel");
      const index = points[p2].getAttribute("index");
      if (newSize === void 0) {
        newSize = w.globals.markers.size[index] + w.config.markers.hover.sizeOffset;
      }
      if (col === parseInt(rel, 10)) {
        me.newPointSize(col, points[p2]);
        const cx = points[p2].getAttribute("cx");
        const cy = points[p2].getAttribute("cy");
...
var Intersect = class {
  ...
  handleHeatTreeTooltip({ e: e2, opt, x, y: y2, type }) {
    ...
    if (e2.target.classList.contains(`apexcharts-${type}-rect`)) {
      const i2 = this.getAttr(e2, "i");
      const j = this.getAttr(e2, "j");
      const cx = this.getAttr(e2, "cx");
      const cy = this.getAttr(e2, "cy");
      const width = this.getAttr(e2, "width");
      const height = this.getAttr(e2, "height");
      ...
      if (ttCtx.w.config.tooltip.followCursor) {
        ...
        y2 = w.interact.clientY - seriesBound.top - (y2 > w.layout.gridHeight / 2 ? ttCtx.tooltipRect.ttHeight : 0);
      }
    }
    return {
      x,
  ...
  handleMarkerTooltip({ e: e2, opt, x, y: y2 }) {
    ...
    if (e2.target.classList.contains("apexcharts-marker")) {
      ...
      w.interact.capturedDataPointIndex = j;
      x = cx;
      y2 = cy + w.layout.translateY - ttCtx.tooltipRect.ttHeight * 1.4;
      if (ttCtx.w.config.tooltip.followCursor) {
        const elGrid = ttCtx.getElGrid();
        const seriesBound = elGrid.getBoundingClientRect();
        y2 = ttCtx.e.clientY + w.layout.translateY - seriesBound.top;
  ...
  handleBarTooltip({ e: e2, opt }) {
    ...
    if (!ttCtx.fixedTooltip && (!w.config.tooltip.shared || w.globals.isBarHorizontal && ttCtx.tooltipUtil.hasBars())) {
      y2 = y2 + w.layout.translateY - ttCtx.tooltipRect.ttHeight / 2;
  ...
  getBarTooltipXY({ e: e2, opt }) {
    ...
    if (cl.contains("apexcharts-bar-area") || cl.contains("apexcharts-candlestick-area") || cl.contains("apexcharts-boxPlot-area") || cl.contains("apexcharts-rangebar-area")) {
      ...
      const y22 = bar.getAttribute("data-range-y2");
      if (w.globals.comboCharts) {
  ...
  nonAxisChartsTooltips({ e: e2, opt, tooltipRect }) {
    ...
    if (e2.type === "mousemove" || e2.type === "touchmove") {
      ...
      if (w.config.tooltip.intersect && arcPath.hasAttribute("data:cx") && arcPath.hasAttribute("data:cy")) {
        ...
        y2 = svgBound.top - seriesBound.top + parseFloat(arcPath.getAttribute("data:cy")) - tooltipRect.ttHeight - 10;
      } else {
        x = w.interact.clientX - seriesBound.left - tooltipRect.ttWidth / 2;
        y2 = w.interact.clientY - seriesBound.top - tooltipRect.ttHeight - 10;
      }
      tooltipEl.style.left = x + "px";
      tooltipEl.style.top = y2 + "px";
...
var Legend = class {
  constructor(w, ctx) {
    this.w = w;
    this.ctx = ctx;
    this.printDataLabelsInner = (...a2) => {
      var _a;
      return (_a = ctx.pie) == null ? void 0 : _a.printDataLabelsInner(...a2);
    };
  ...
  onLegendHovered(e2) {
    ...
    if (w.config.chart.type !== "heatmap" && !this.isBarsDistributed) {
      ...
      if (hoverOverLegend) {
        const seriesCnt = parseInt(e2.target.getAttribute("rel"), 10) - 1;
  ...
  onLegendKeyDown(e2) {
    ...
    if (e2.key === "Enter" || e2.key === " ") {
      e2.preventDefault();
      const rel = e2.target.getAttribute("rel");
      me.onLegendClick(e2);
  ...
  onLegendClick(e2) {
    ...
    if (e2.target.classList.contains("apexcharts-legend-series") || e2.target.classList.contains("apexcharts-legend-text") || e2.target.classList.contains("apexcharts-legend-marker")) {
      const seriesCnt = parseInt(e2.target.getAttribute("rel"), 10) - 1;
      const isHidden = e2.target.getAttribute("data:collapsed") === "true";
...
var Toolbar = class {
  ...
  addToolbarEventListeners() {
    ...
    this.elMenuIcon.addEventListener("keydown", (e2) => {
...
var Helpers$2 = class Helpers3 {
  ...
  getY1Y2(type, anno) {
    var _a;
    const w = this.w;
    const y2 = type === "y1" ? anno.y : anno.y2;
    let yP;
    let clipped = false;
    if (this.annoCtx.invertAxis) {
      ...
      const yPos = w.config.yaxis[anno.yAxisIndex].logarithmic ? new CoreUtils(this.w).getLogVal(
        ...
        y2,
        seriesIndex
      ) / w.globals.yLogRatio[seriesIndex] : (y2 - w.globals.minYArr[seriesIndex]) / (w.globals.yRange[seriesIndex] / w.layout.gridHeight);
...
var KeyboardNavigation = class {
  ...
  _showTooltipBar(i2, j, ttCtx) {
    ...
    const rangeData = (_d = (_c = (_b = (_a = w.rangeData.seriesRange) == null ? void 0 : _a[i2]) == null ? void 0 : _b[j]) == null ? void 0 : _c.y) == null ? void 0 : _d[0];
    ...
    if (w.globals.isBarHorizontal) {
      ...
      if (barDomEl) {
        ...
        const y2 = barCy + bh / 2 - ttHeight / 2;
        let x = barCx + bw;
  ...
  _showTooltipRadialBar(i2, _j, ttCtx, tooltipEl) {
    ...
    if (arcEl) {
      ...
      const y2 = centroid.y + (w.layout.translateY || 0);
      tooltipEl.style.left = x - ttWidth / 2 + "px";
      tooltipEl.style.top = y2 - ttHeight - 10 + "px";
    }
  }
  ...
  _showTooltipHeatTree(i2, j, ttCtx, tooltipEl, type) {
    ...
    if (cell) {
      ...
      const y2 = cellCy + cellHeight / 2 - ttHeight / 2;
...
var BarDataLabels = class {
  ...
  handleBarDataLabels(opts) {
    ...
    if (w.config.dataLabels.enabled) {
      const yLabel = w.seriesData.series[i2][j];
    ...
    dataLabels = this.drawCalculatedDataLabels({
      ...
      val: this.barCtx.isRangeBar ? [y1, y22] : w.config.chart.stackType === "100%" ? series[realIndex][j] : w.seriesData.series[realIndex][j],
      i: realIndex,
      j,
      barWidth,
      barHeight,
  ...
  calculateColumnsDataLabelsPosition(opts) {
    ...
    const valIsNegative = w.seriesData.series[i2][j] < 0;
    let newY = y2;
...
var Helpers$1 = class Helpers4 {
  ...
  getPathFillColor(series, i2, j, realIndex) {
    ...
    let fillColor = null;
    const seriesNumber = this.barCtx.barOptions.distributed ? j : i2;
  ...
  getColumnPaths({
    ...
    const direction = (series[i2][j] >= 0 ? 1 : -1) * (isReversed ? -1 : 1);
  ...
  getGoalValues(type, zeroW, zeroH, i2, j, translationsIndex) {
    ...
    if (w.seriesData.seriesGoals[i2] && w.seriesData.seriesGoals[i2][j] && Array.isArray(w.seriesData.seriesGoals[i2][j])) {
      w.seriesData.seriesGoals[i2][j].forEach((goal) => {
    ...
    if (this.barCtx.barOptions.isDumbbell && w.rangeData.seriesRange.length) {
      ...
      pushGoal(w.rangeData.seriesRangeStart[i2][j], commonAttrs);
      pushGoal(w.rangeData.seriesRangeEnd[i2][j], __spreadProps(__spreadValues({}, commonAttrs), {
      ...
      if (Array.isArray(goalX)) {
        goalX.forEach((goal) => {
          if (goal.x >= -1 && goal.x <= graphics.w.layout.gridWidth + 1) {
            ...
            const y2 = barYPosition + sHeight + barHeight / 2;
...
var Bar = class {
  ...
  draw(series, seriesIndex) {
    ...
    for (let i2 = 0, bc = 0; i2 < series.length; i2++, bc++) {
      ...
      for (let j = 0; j < series[i2].length; j++) {
        ...
        const pathFill = this.barHelpers.getPathFillColor(series, i2, j, realIndex);
      ...
      let fetchColor = function(i22) {
        ...
        if (Array.isArray(exp) && exp.length > 0) {
          ...
          if (typeof c2 === "function") {
            return c2({
              value: w.seriesData.series[i22][j],
              dataPointIndex: j,
              w
            });
          }
  ...
  drawBarPaths({
    ...
    let barYPosition;
    if (w.axisFlags.isXNumeric) {
      y2 = (w.seriesData.seriesX[i2][j] - w.globals.minX) / this.invertedXRatio - barHeight;
...
var BarStacked = class extends Bar {
  draw(series, seriesIndex) {
    ...
    for (let i2 = 0, bc = 0; i2 < series.length; i2++, bc++) {
      ...
      const initPositions = this.initialPositions(x, y2, void 0, void 0, void 0, void 0, translationsIndex);
      const {
        xDivision,
        // xDivision is the GRIDWIDTH divided by number of datapoints (columns)
        yDivision,
  ...
  drawStackedBarPaths({
    ...
    const w = this.w;
    const barYPosition = y2 + columnGroupIndex * barHeight;
    let barXPosition;
    const i2 = indexes.i;
    const j = indexes.j;
    const realIndex = indexes.realIndex;
...
var BoxCandleStick = class extends Bar {
  ...
  drawHorizontalBoxPaths({
    ...
    let m = ohlc.m;
    if (w.axisFlags.isXNumeric) {
      y2 = (w.seriesData.seriesX[realIndex][j] - w.globals.minX) / this.invertedXRatio - barHeight / 2;
    }
  ...
  getOHLCValue(i2, j) {
    ...
    const o2 = getCandleVal(w.candleData.seriesCandleO);
    const m = getCandleVal(w.candleData.seriesCandleM);
    const c2 = getCandleVal(w.candleData.seriesCandleC);
    const l2 = getCandleVal(w.candleData.seriesCandleL);
    return {
...
var TreemapHelpers = class {
  ...
  determineColor(chartType, i2, j) {
    const w = this.w;
    const val = w.seriesData.series[i2][j];
    ...
    let foreColor = null;
    let min = Math.min(...w.seriesData.series[i2]);
    let max = Math.max(...w.seriesData.series[i2]);
  ...
  calculateDataLabels({ text, x, y: y2, i: i2, j, colorProps, fontSize }) {
    ...
    if (dataLabelsConfig.enabled) {
      ...
      const dataLabelsY = y2 + parseFloat(dataLabelsConfig.style.fontSize) / 3 + offY;
      dataLabels.plotDataLabelsText({
        x: dataLabelsX,
        y: dataLabelsY,
        text,
  ...
  draw(series) {
    ...
    for (let i2 = rev ? 0 : heatSeries.length - 1; rev ? i2 < heatSeries.length : i2 >= 0; rev ? i2++ : i2--) {
      const elSeries = graphics.group({
        ...
        seriesName: Utils$1.escapeString(w.seriesData.seriesNames[i2]),
        rel: i2 + 1,
        "data:realIndex": i2
      });
      ...
      for (let dIndex = 0; dIndex < w.globals.dataPoints; dIndex++) {
        if (w.seriesData.seriesX.length && !w.globals.allSeriesHasEqualX) {
          if (w.globals.minX + w.globals.minXDiff * dIndex < w.seriesData.seriesX[i2][j]) {
            x1 = x1 + xDivision;
            continue;
          }
        }
        ...
        const formattedText = formatter(w.seriesData.series[i2][j], {
...
var Line = class {
  ...
  _initSerieVariables(series, i2, realIndex) {
    ...
    const longestSeries = series[i2].length === w.globals.dataPoints;
    this.elSeries.attr({
  ...
  _handleMarkersAndLabels({ type, pointsPos, isRangeStart, i: i2, j, realIndex }) {
    ...
    const dataLabels = new DataLabels(this.w, this.ctx);
    if (!this.pointsChart) {
      if (w.seriesData.series[i2].length > 1) {
          ...
          switch (pathState) {
            case 0:
              ...
              if (j < series[i2].length - 2) {
                const p2 = graphics.curve(pX + length, pY, x - length, y2, x, y2);
                linePath += p2;
                areaPath += p2;
                break;
              }
        ...
        if (series[i2][j] === null) {
          ...
          switch (pathState) {
            case 0:
              ...
              pathState = 1;
              if (j < series[i2].length - 2) {
                const p2 = pathToPoint(curve, x, y2);
                linePath += p2;
                areaPath += p2;
                break;
              }
...
var Pie = class {
  ...
  draw(series) {
    ...
    for (let i2 = 0; i2 < series.length; i2++) {
      const angle = this.fullAngle * Utils$1.negToZero(series[i2]) / total;
  ...
  drawArcs(sectorAngleArr, series) {
    ...
    for (let i2 = 0; i2 < sectorAngleArr.length; i2++) {
      ...
      if (typeof w.interact.selectedDataPoints[0] !== "undefined" && w.interact.selectedDataPoints[0].indexOf(i2) > -1) {
        this.pieClicked(i2);
      }
      if (w.config.dataLabels.enabled) {
  ...
  pieClicked(i2) {
    const w = this.w;
    const me = this;
    const size = me.sliceSizes[i2] + (w.config.plotOptions.pie.expandOnClick ? 4 : 0);
    const elPath = w.dom.Paper.findOne(
    ...
    if (elPath.attr("data:pieClicked") === "true") {
      ...
      w.interact.capturedDataPointIndex = i2;
  ...
  renderInnerDataLabels(dataLabelsGroup, dataLabelsConfig, opts) {
    ...
    const y2 = !this.donutDataLabels.total.label ? opts.centerY - opts.centerY / 6 : opts.centerY;
...
var Radar = class {
  ...
  draw(series) {
    ...
    series.forEach((s2, i2) => {
      const longestSeries = s2.length === w.globals.dataPoints;
      ...
      s2.forEach((sj, j) => {
        ...
        const dataLabelsConfig = w.config.dataLabels;
        if (dataLabelsConfig.enabled) {
          const text = dataLabelsConfig.formatter(w.seriesData.series[i2][j], {
            seriesIndex: i2,
            dataPointIndex: j,
            w
          });
...
var Radial = class extends Pie {
  ...
  drawArcs(opts) {
    ...
    for (let i2 = reverseLoop ? opts.series.length - 1 : 0; reverseLoop ? i2 >= 0 : i2 < opts.series.length; reverseLoop ? i2-- : i2++) {
      ...
      Series.addCollapsedClassToSeries(this.w, elRadialBarArc, i2);
      opts.size = opts.size - strokeWidth - this.margin;
      const pathFill = fill.fillPath({
        seriesNumber: i2,
        size: opts.size,
      ...
      if (this.barLabels.enabled) {
        ...
        const text = this.barLabels.formatter(w.seriesData.seriesNames[i2], {
          seriesIndex: i2,
          w
        });
  ...
  onBarLabelClick(e2) {
    const seriesIndex = parseInt(e2.target.getAttribute("rel"), 10) - 1;
...
var RangeBar = class extends Bar {
    ...
    for (let i2 = 0; i2 < series.length; i2++) {
      ...
      for (let j = 0; j < w.globals.dataPoints; j++) {
        ...
        if (this.isHorizontal) {
          ...
          if (w.axisFlags.isXNumeric) {
            x = (w.seriesData.seriesX[i2][j] - w.globals.minX) / this.xRatio - barWidth / 2;
          }
          barXPosition = x + barWidth * this.visibleI;
  ...
  detectOverlappingBars({
    ...
    const w = this.w;
    let overlaps = [];
    const rangeName = w.config.series[i2].data[j].rangeName;
    ...
    if (this.isHorizontal) {
      ...
      if (overlappedIndex > -1 && !w.config.plotOptions.bar.rangeBarOverlap) {
        overlaps = w.rangeData.seriesRange[i2][overlappedIndex].overlaps;
  ...
  drawRangeBarPaths({
    ...
    const x2 = this.preventBarOverflow(zeroW + y22 / this.invertedYRatio);
    const range = this.getRangeValue(realIndex, j);
  ...
  getRangeValue(i2, j) {
    const w = this.w;
    return {
      start: w.rangeData.seriesRangeStart[i2][j],
      end: w.rangeData.seriesRangeEnd[i2][j]
    };
  }
};
