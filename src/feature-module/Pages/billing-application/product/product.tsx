...
const Product: any = () => {
    ...
    const [openingStockRows, setOpeningStockRows] = useState([
        { id: 1, location: { value: 'head-office', label: 'Head Office' }, openingStock: '', valuePerUnit: '' }
    ]);
    ...
    useEffect(() => {
        ...
        const h = (e: MouseEvent) => {
            ...
            if (!t.closest('.gear-menu-wrapper')) setShowGearMenu(false);
        };
    ...
    const computeSku = (nameVal: string, varOps: string[], attrs: Attribute[], fmtList: typeof skuFormat) => {
        ...
        fmtList.forEach((fmt, i) => {
            if (!fmt.label) return;
            const base = fmt.label === 'Item Name' ? (nameVal || 'ITEM') : fmt.label === 'Custom Text' ? (fmt.customValue || '') : (() => { const idx = attrs.findIndex(a => a.name === fmt.label); return (idx !== -1 && varOps[idx]) ? varOps[idx] : 'ATTR'; })();
            if (!base) return;
            let v = base;
    ...
    return (
        <div className="page-wrapper">
                        ...
                        <div className="card mb-0">
                            <div className="card-body">
                                ...
                                <ul className="nav nav-tabs mb-4">
                                    {([['overview','Overview'],['locations','Locations'],['transactions','Transactions'],['history','History']] as const).map(([key,label]) => (
                                        <li key={key} className="nav-item">
                                            <button type="button" className={`nav-link px-3 py-2 ${activeTab === key ? 'active fw-semibold' : 'text-muted'}`} onClick={() => setActiveTab(key)}>{label}</button>
                                        </li>
                                    ))}
                                </ul>
                                {activeTab === 'overview' && <form onSubmit={(e) => e.preventDefault()}>
                                ...
                                {activeTab === 'locations' && (() => {
                                    const totalStock = openingStockRows.reduce((s, r) => s + (parseFloat(r.openingStock) || 0), 0);
                                    const fmt = (n: number) => n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                                    return (
                                        <div className="py-2">
                                            {/* Header row */}
                                            <div className="d-flex align-items-center justify-content-between mb-3">
                                                <h6 className="mb-0 fw-semibold">Stock Locations</h6>
                                                <div className="d-flex align-items-center gap-2">
                                                    <div className="position-relative gear-menu-wrapper">
                                                        <button type="button" className="btn btn-sm btn-light border d-flex align-items-center gap-1 px-2 py-1" onClick={() => setShowGearMenu(v => !v)}>
                                                            <i className="ti ti-settings fs-14" />
                                                            <i className="ti ti-chevron-down fs-11" />
                                                        </button>
                                                        {showGearMenu && (
                                                            <div className="position-absolute start-0 mt-1 bg-white border rounded shadow" style={{ zIndex: 200, minWidth: '200px', top: '100%' }}>
                                                                {/* Opening Stock */}
                                                                <button type="button"
                                                                    className="d-flex align-items-center gap-2 w-100 px-3 py-2 border-0 bg-transparent text-start"
                                                                    style={{ fontSize: '13px', color: '#333', cursor: 'pointer' }}
                                                                    onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#f5f5f5')}
                                                                    onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
                                                                    onClick={() => { setShowGearMenu(false); setShowOpeningStockModal(true); }}>
                                                                    <span style={{ width: '20px', height: '20px', backgroundColor: '#fdecea', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                                        <i className="ti ti-package" style={{ fontSize: '12px', color: '#e41f07' }} />
                                                                    </span>
                                                                    <span className="fw-medium">Opening Stock</span>
                                                                </button>
                                                                {/* Manage Locations */}
                                                                <button type="button"
                                                                    className="d-flex align-items-center gap-2 w-100 px-3 py-2 border-0 bg-transparent text-start"
                                                                    ...
                                                                    <span style={{ width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                                        <i className="ti ti-settings" style={{ fontSize: '13px', color: '#888' }} />
                                                                    </span>
                                                                    <span className="fw-medium">Manage Locations</span>
                                                                </button>
                                                            </div>
                                                        )}
                                                    </div>
                                                    <div className="btn-group btn-group-sm">
                                                        <button type="button" className={`btn ${stockMode === 'accounting' ? 'btn-primary' : 'btn-outline-secondary'}`} onClick={() => setStockMode('accounting')}>Accounting Stock</button>
                                                        <button type="button" className={`btn ${stockMode === 'physical' ? 'btn-primary' : 'btn-outline-secondary'}`} onClick={() => setStockMode('physical')}>Physical Stock</button>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="table-responsive border rounded mb-4">
                                                <table className="table table-nowrap mb-0">
                                                    <thead>
                                                        <tr style={{ backgroundColor: '#f8f9fa', fontSize: '11px', letterSpacing: '0.5px' }}>
                                                            <th className="text-uppercase fw-semibold ps-3 border-bottom-0 align-middle" style={{ width: '35%', color: '#e41f07' }}>Location Name</th>
                                                            <th className="text-uppercase fw-semibold text-center border-bottom-0" colSpan={3} style={{ color: '#555' }}>
                                                                {stockMode === 'physical' ? 'Physical Stock' : 'Accounting Stock'}
                                                            </th>
                                                        </tr>
                                                        <tr style={{ backgroundColor: '#f8f9fa', fontSize: '11px', letterSpacing: '0.5px' }}>
                                                            <th className="ps-3 border-top-0" style={{ width: '35%' }}></th>
                                                            <th className="text-uppercase fw-semibold text-end border-top-0" style={{ color: '#777' }}>Stock on Hand</th>
                                                            <th className="text-uppercase fw-semibold text-end border-top-0" style={{ color: '#777' }}>Committed Stock</th>
                                                            <th className="text-uppercase fw-semibold text-end pe-3 border-top-0" style={{ color: '#777' }}>Available for Sale</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {openingStockRows.map(row => {
                                                            ...
                                                            return (
                                                                <tr key={row.id}>
                                                                    <td className="ps-3">
                                                                        <span className="fw-medium">{row.location?.label || '—'}</span>
                                                                        {row.location?.value === 'head-office' && <span className="ms-1" style={{ fontSize: '13px' }}>⭐</span>}
                                                                    </td>
                                                                    <td className="text-end fw-medium">{fmt(qty)}</td>
                                                                    <td className="text-end fw-medium" style={{ color: '#e41f07' }}>0.00</td>
                                                                    <td className="text-end pe-3 fw-medium" style={{ color: '#2563eb' }}>{fmt(qty)}</td>
                                                                </tr>
                                                            );
                                                        })}
                                                    </tbody>
                                            ...
                                            <div className="row g-4">
                                                <div className="col-md-6">
                                                    <p className="text-uppercase fw-bold mb-2" style={{ fontSize: '12px', letterSpacing: '0.5px', color: '#333' }}>Accounting Stock</p>
                                                    <div className="d-flex justify-content-between border-bottom py-2">
                                                        <span className="text-muted fs-13">Stock on Hand</span>
                                                        <span className="fw-bold fs-13">{fmt(totalStock)}</span>
                                                    </div>
                                                    <div className="d-flex justify-content-between border-bottom py-2">
                                                        <span className="text-muted fs-13">Committed Stock</span>
                                                        <span className="fw-medium fs-13">0.00</span>
                                                    </div>
                                                    <div className="d-flex justify-content-between py-2">
                                                        <span className="text-muted fs-13">Available for Sale</span>
                                                        <span className="fw-medium fs-13">{fmt(totalStock)}</span>
                                                    </div>
                                                </div>
                                                <div className="col-md-6">
                                                    <p className="text-uppercase fw-bold mb-2" style={{ fontSize: '12px', letterSpacing: '0.5px', color: '#333' }}>Physical Stock</p>
                                                    <div className="d-flex justify-content-between border-bottom py-2">
                                                        <span className="text-muted fs-13">Stock on Hand</span>
                                                        <span className="fw-medium fs-13">{fmt(totalStock)}</span>
                                                    </div>
                                ...
                                {activeTab === 'transactions' && (
                                    <div className="text-center text-muted py-5">No transactions yet.</div>
                                )}

                                {activeTab === 'history' && (
                                    <div className="text-center text-muted py-5">No history yet.</div>
                                )}
                            </div>
                        </div>
                    </div>
            ...
            {showOpeningStockModal && (
                <div className="modal show d-block" style={{ background: 'rgba(0,0,0,0.5)', zIndex: 1055 }} onClick={(e) => { if (e.target === e.currentTarget) setShowOpeningStockModal(false); }}>
                    <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '660px' }}>
                        <div className="modal-content" style={{ borderRadius: 6, boxShadow: '0 8px 32px rgba(0,0,0,0.2)', border: '1px solid #d0d0d0', overflow: 'hidden' }}>
                            ...
                            {/* Title bar — white, compact */}
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px', background: '#fff', borderBottom: '1px solid #e0e0e0' }}>
                                <span style={{ fontSize: 13, fontWeight: 600, color: '#333' }}>{itemName || 'Product'}</span>
                                <button type="button" onClick={() => setShowOpeningStockModal(false)}
                                    style={{ background: 'none', border: 'none', fontSize: 17, color: '#666', cursor: 'pointer', lineHeight: 1, padding: '0 2px' }}>
                                    ×
                                </button>
                            </div>

                            {/* Table with side gaps + border lines */}
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', borderLeft: '1px solid #e0e0e0', borderRight: '1px solid #e0e0e0', marginTop: 12, marginBottom: 8 }}>
                                    <thead>
                                        <tr style={{ borderBottom: '1px solid #e0e0e0' }}>
                                            ...
                                            <th style={{ width: '27%', padding: '6px 8px', color: '#e41f07', textAlign: 'right', borderRight: '1px solid #e0e0e0' }}>
                                                ...
                                                <button type="button"
                                                    ...
                                                    onClick={() => { const v = openingStockRows[0]?.openingStock || ''; setOpeningStockRows(r => r.map(x => ({ ...x, openingStock: v }))); }}>
                                                    Copy To All
                                                </button>
                                            </th>
                                            <th style={{ width: '31%', padding: '6px 8px', color: '#e41f07', textAlign: 'right', borderRight: '1px solid #e0e0e0' }}>
                                                ...
                                                <button type="button"
                                                    style={{ background: 'none', border: 'none', padding: 0, fontSize: 11, color: '#e41f07', cursor: 'pointer', fontWeight: 500 }}
                                                    onClick={() => { const v = openingStockRows[0]?.valuePerUnit || ''; setOpeningStockRows(r => r.map(x => ({ ...x, valuePerUnit: v }))); }}>
                                                    Copy To All
                                                </button>
                                            </th>
                                            <th style={{ width: '6%', padding: '6px 0' }} />
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {openingStockRows.map(row => (
                                            <tr key={row.id} style={{ borderBottom: '1px solid #efefef' }}>
                                                <td style={{ padding: '10px 8px', verticalAlign: 'middle', borderRight: '1px solid #efefef' }}>
                                                    <Select
                                                        ...
                                                        value={row.location}
                                                        onChange={(val) => setOpeningStockRows(rows => rows.map(r => r.id === row.id ? { ...r, location: val as any } : r))}
                                                        styles={{
                                                            control: (base) => ({ ...base, border: '1px solid #d9d9d9', boxShadow: 'none', minHeight: '30px', fontSize: '13px', cursor: 'pointer', borderRadius: '3px', backgroundColor: 'white' }),
                                                            indicatorSeparator: () => ({ display: 'none' }),
                                                            dropdownIndicator: (base) => ({ ...base, padding: '0 6px', color: '#888' }),
                                                            singleValue: (base) => ({ ...base, color: '#333', fontWeight: 500 }),
                                                            menu: (base) => ({ ...base, fontSize: '13px', zIndex: 9999 }),
                                                            option: (base, state) => ({ ...base, backgroundColor: state.isSelected ? '#e41f07' : state.isFocused ? '#fff3f2' : 'white', color: state.isSelected ? '#fff' : '#333', cursor: 'pointer' }),
                                                        }}
                                                ...
                                                <td style={{ padding: '8px 8px', verticalAlign: 'middle', borderRight: '1px solid #efefef' }}>
                                                    <input type="number" value={row.openingStock}
                                                        onChange={(e) => setOpeningStockRows(rows => rows.map(r => r.id === row.id ? { ...r, openingStock: e.target.value } : r))}
                                                ...
                                                <td style={{ padding: '8px 8px', verticalAlign: 'middle', borderRight: '1px solid #efefef' }}>
                                                    <input type="number" value={row.valuePerUnit}
                                                        onChange={(e) => setOpeningStockRows(rows => rows.map(r => r.id === row.id ? { ...r, valuePerUnit: e.target.value } : r))}
                                                        style={{ width: '100%', textAlign: 'right', border: '1px solid #d9d9d9', borderRadius: 3, padding: '4px 8px', fontSize: 13, outline: 'none', color: '#333', background: '#fff' }} />
                                                </td>
                                                <td style={{ padding: '8px 4px', verticalAlign: 'middle', textAlign: 'center' }}>
                                                    <button type="button" style={{ background: 'none', border: 'none', padding: '2px 4px', cursor: 'pointer', color: '#e41f07', lineHeight: 1 }}
                                                        onClick={() => setOpeningStockRows(rows => rows.length > 1 ? rows.filter(r => r.id !== row.id) : rows)}>
