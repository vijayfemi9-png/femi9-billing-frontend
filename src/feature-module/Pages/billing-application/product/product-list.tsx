...
type Item = {
    ...
    locationStock?: Record<number, number>;
};
...
const ProductList = () => {
    ...
    const [locStockView, setLocStockView] = useState<"accounting" | "physical">("accounting");
    const [locSettingsOpen, setLocSettingsOpen] = useState(false);
    const locSettingsRef = React.useRef<HTMLDivElement>(null);
    const [showOpeningStockModal, setShowOpeningStockModal] = useState(false);
    const [openingStockForm, setOpeningStockForm] = useState<Record<number, string>>({});
    const [openingStockValueForm, setOpeningStockValueForm] = useState<Record<number, string>>({});
    ...
    useEffect(() => {
        const handler = (e: MouseEvent) => {
            ...
            if (locSettingsRef.current && !locSettingsRef.current.contains(e.target as Node)) setLocSettingsOpen(false);
        };
        document.addEventListener("mousedown", handler);
        return () => document.removeEventListener("mousedown", handler);
    }, []);
    ...
    const handleSaveOpeningStock = () => {
        ...
        const newLocationStock: Record<number, number> = {};
        locations.forEach(loc => {
            const val = parseFloat(openingStockForm[loc.id] || "0");
            newLocationStock[loc.id] = isNaN(val) ? 0 : val;
        });
        const totalStock = Object.values(newLocationStock).reduce((s, v) => s + v, 0);
        const nowStr = fmtDate(new Date());
        const user = getCurrentUser();
        const all: Item[] = JSON.parse(localStorage.getItem(PRODUCT_STORAGE_KEY) || '[]');
        const base = all.length ? all : SEED_PRODUCTS;
        const updated = base.map(d => {
            if (d.id !== activeItem.id) return d;
            const prev = d.history || [];
            return {
                ...d,
                locationStock: newLocationStock,
                stockOnHand: totalStock,
                history: [...prev, { date: nowStr, details: 'Opening stock adjusted by', user }],
            };
        });
        ...
        <>
            ...
            <div className="page-wrapper" style={{ overflow: 'hidden' }}>
                <div className="content pb-0" style={{ display: "flex", flexDirection: "column", height: '100vh', overflow: 'hidden' }}>
                    ...
                    {isSplitView ? (
                            ...
                            <div className="d-flex" style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
                                ...
                                <div style={{ width: 320, borderRight: "1px solid #e3e3e3", display: isMobile ? "none" : "flex", flexDirection: "column", background: "#fff", flexShrink: 0, overflow: "hidden" }}>
                                    <div className="d-flex align-items-center gap-2 px-3 py-2 border-bottom">
                                        <span className="fw-semibold fs-15">{currentViewLabel} <span className="text-muted fs-12">({filteredItems.length})</span></span>
                                                        ...
                                                        <div className="d-flex flex-column" style={{ gap: isMobile ? 20 : 32 }}>
                                                            ...
                                                            <div style={{ border: "1px solid #e8e8e8", borderRadius: 8, background: "#fff", overflow: "hidden" }}>
                                                                ...
                                                                <div style={{ padding: "12px 16px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, background: "#fafafa" }}>
                                                                    <span style={{ fontSize: 13, fontWeight: 600, color: "#333" }}>Sales Order Summary <span style={{ fontWeight: 400, color: "#999" }}>(In ₹)</span></span>
                                                    ...
                                                    {activeTab === "Locations" && (
                                                        <div className="p-3">
                                                            ...
                                                            <div className="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                                                                <div className="d-flex align-items-center gap-2">
                                                                    <span className="fw-bold fs-14 text-dark">Stock Locations</span>
                                                                    <div ref={locSettingsRef} className="position-relative">
                                                                        <button
                                                                            className="btn btn-light btn-sm d-flex align-items-center gap-1 px-2 py-1 border"
                                                                            style={{ fontSize: 12, borderRadius: 4 }}
                                                                            onClick={() => setLocSettingsOpen(o => !o)}
                                                                        >
                                                                            <i className="ti ti-settings fs-13 text-muted" />
                                                                            <i className={`ti ${locSettingsOpen ? "ti-chevron-up" : "ti-chevron-down"} fs-11 text-muted`} />
                                                                        </button>
                                                                        {locSettingsOpen && (
                                                                            <div className="card border shadow-sm position-absolute mt-1" style={{ minWidth: 200, zIndex: 400, top: "100%", left: 0 }}>
                                                                                <div
                                                                                    className="px-3 py-2 d-flex align-items-center gap-2 fs-13"
                                                                                    ...
                                                                                    onClick={() => {
                                                                                        ...
                                                                                        locations.forEach(loc => {
                                                                                            init[loc.id] = ((activeItem?.locationStock?.[loc.id]) ?? 0).toString();
                                                                                        });
                                                                                        setOpeningStockForm(init);
                                                                                        setShowOpeningStockModal(true);
                                                                                        setLocSettingsOpen(false);
                                                                                    }}
                                                                                >
                                                                                    <i className="ti ti-package-import text-primary fs-14" />
                                                                                    Opening Stock
                                                                                </div>
                                                                                ...
                                                                                <div
                                                                                    ...
                                                                                    onMouseEnter={e => (e.currentTarget.style.background = "#f5f5f5")}
                                                                                    onMouseLeave={e => (e.currentTarget.style.background = "")}
                                                                                    onClick={() => { navigate(all_routes.locations); setLocSettingsOpen(false); }}
                                                                                >
                                                                                    <i className="ti ti-settings text-muted fs-14" />
                                                                                    Manage Locations
                                                                                </div>
                                                                            </div>
                                                                        )}
                                                            ...
                                                            <div className="table-responsive border rounded-3 overflow-hidden">
                                                                <table className="table table-nowrap mb-0" style={{ fontSize: 13 }}>
                                                                    <thead>
                                                                        ...
                                                                        <tr style={{ background: "#f5f7fa", borderTop: "1px solid #e3e3e3" }}>
                                                                            <th className="py-1 px-3 border-0" style={{ width: "34%" }} />
                                                                            <th className="py-1 px-3 fw-semibold border-0 text-end" style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: 0.5 }}>Stock on Hand</th>
                                                                            <th className="py-1 px-3 fw-semibold border-0 text-end" style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: 0.5 }}>Committed Stock</th>
                                                                            <th className="py-1 px-3 fw-semibold border-0 text-end" style={{ fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: 0.5 }}>Available for Sale</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody>
                                                                        {locations.length > 0 ? locations.map(loc => {
                                                                            const locStock = activeItem?.locationStock?.[loc.id] ?? (loc.isDefault ? (activeItem?.stockOnHand ?? 0) : 0);
                                                                            const committed = 0;
                                                                            const available = Math.max(0, locStock - committed);
                                                                            return (
                                                                                <tr key={loc.id} className="border-bottom" style={{ cursor: "default" }}>
                                                                                    <td className="py-3 px-3 border-0">
                                                                                        <div className="d-flex align-items-center gap-2">
                                                                                            <span className="fw-medium text-dark">{loc.name}</span>
                                                                                            {loc.isDefault && <span style={{ color: "#f59e0b", fontSize: 14 }}>★</span>}
                                                                                        </div>
                                                                                    </td>
                                                                                    <td className="py-3 px-3 border-0 text-end fw-medium text-dark">{locStock.toFixed(2)}</td>
                                                                                    <td className="py-3 px-3 border-0 text-end text-muted">{committed.toFixed(2)}</td>
                                                                                    <td className="py-3 px-3 border-0 text-end text-dark">{available.toFixed(2)}</td>
                                                                                </tr>
                                                                            );
                                                                        }) : (
                                                                            <tr>
                                                                                <td colSpan={4} className="py-5 text-center text-muted" style={{ cursor: "pointer" }} onClick={() => navigate(all_routes.locations)}>
                                                                                    <i className="ti ti-map-pin fs-32 mb-2 d-block opacity-25" />
                                                                                    <div className="fs-13">No locations found. Click to add locations.</div>
                                                                                </td>
                                                                            </tr>
                                                                        )}
                                                                    </tbody>
                        ...
                        <div className="card border-0 rounded-0" style={{ flex: 1, minHeight: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            ...
            {showOpeningStockModal && (
                <div
                    ...
                    onClick={(e) => { if (e.target === e.currentTarget) setShowOpeningStockModal(false); }}
                >
                    <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: 680 }}>
                        <div className="modal-content" style={{ borderRadius: 6, boxShadow: '0 8px 32px rgba(0,0,0,0.2)', border: '1px solid #d0d0d0', overflow: 'hidden' }}>
                            ...
                            {/* Footer */}
                            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, padding: '10px 20px', borderTop: '1px solid #e8e8e8', background: '#fff' }}>
                                <button
                                    type="button"
                                    onClick={() => setShowOpeningStockModal(false)}
                                    style={{ padding: '6px 22px', fontSize: 13, border: '1px solid #ccc', borderRadius: 4, background: '#fff', color: '#555', cursor: 'pointer', fontWeight: 500 }}
                                >
                                    Cancel
                                </button>
                                <button
                                    type="button"
                                    onClick={() => { handleSaveOpeningStock(); setShowOpeningStockModal(false); }}
                                    style={{ padding: '6px 22px', fontSize: 13, border: 'none', borderRadius: 4, background: '#e41f07', color: '#fff', cursor: 'pointer', fontWeight: 600 }}
                                >
                                    Save
                                </button>
                            </div>
