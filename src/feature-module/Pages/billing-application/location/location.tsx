...
//  Shared Styles 
const inp: CSSProperties = { border: "1px solid #e3e3e3", borderRadius: 6, padding: "7px 11px", fontSize: 13, outline: "none", width: "100%", color: "#333" };
const lbl = (req = false): CSSProperties => ({ fontSize: 13, fontWeight: 500, marginBottom: 5, display: "block", color: req ? "#e41f07" : "#444" });
const thStyle: CSSProperties = { padding: "14px 16px", fontWeight: 700, color: "#1e293b", fontSize: 14, textTransform: "capitalize", background: "transparent", borderBottom: "1px solid #f1f5f9", whiteSpace: "nowrap", userSelect: "none" };
const tdStyle: CSSProperties = { padding: "14px 16px", fontSize: 13, color: "#475569", verticalAlign: "middle" };
...
const LocationModal = ({ show, onClose, onSave, editData, seriesList, businessLocations, onAddSeries, showSeriesModal: propShowSeriesModal, setShowSeriesModal: propSetShowSeriesModal, onSaveNewSeries, locationNamesForSeries, renderMode = "modal" }: {
    ...
    const set = <K extends keyof Omit<ProductLocation, "id">>(k: K, v: Omit<ProductLocation, "id">[K]) => setForm(f => ({ ...f, [k]: v }));
    ...
    const shellStyle: CSSProperties = isPageMode
    ...
    const headerStyle: CSSProperties = isPageMode
                                ...
                                {parentDropOpen && (
                                    <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#fff", border: "1px solid #e3e3e3", borderRadius: 8, boxShadow: "0 4px 20px rgba(0,0,0,0.12)", zIndex: 400, overflow: "hidden" }}>
                                        <div style={{ padding: "8px 10px", borderBottom: "1px solid #f0f0f0" }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: 6, background: "#f7f7f7", borderRadius: 6, padding: "5px 10px" }}>
                                                <i className="ti ti-search" style={{ fontSize: 13, color: "#aaa" }} />
                                                <input autoFocus placeholder="Search" style={{ border: "none", background: "none", outline: "none", fontSize: 13, width: "100%", color: "#333" }} id="parent-loc-search" />
                                            </div>
                                        </div>
                                        <div style={{ maxHeight: 200, overflowX: "hidden", overflowY: "auto" }}>
                                ...
                                {rules.map((rule, i) => (
                                    <tr key={rule.id} style={{ borderBottom: i < rules.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                                        <td style={{ padding: "8px 16px", verticalAlign: "middle" }}>
                                            <select style={{ ...inp, height: 36, fontSize: 13, width: "100%" }} value={rule.type} onChange={e => updateRule(rule.id, { type: e.target.value as GeneratorRule["type"] })}>
                                        ...
                                        <td style={{ padding: "8px 16px", verticalAlign: "middle" }}>
                                            {rule.type === "Module Name" ? (
                                                <div style={{ display: "flex", gap: 6 }}>
                                                    <select style={{ ...inp, height: 36, fontSize: 13, flex: 1 }} value={rule.show} onChange={e => updateRule(rule.id, { show: e.target.value as GeneratorRule["show"] })}>
                                                        <option>First</option>
                                                        <option>Last</option>
                                                        <option>Full</option>
                                                    </select>
                                        ...
                                        <td style={{ padding: "8px 16px", verticalAlign: "middle" }}>
                                            <select style={{ ...inp, height: 36, fontSize: 13, width: "100%" }} value={rule.letterCase} onChange={e => updateRule(rule.id, { letterCase: e.target.value as GeneratorRule["letterCase"] })}>
                                    ...
                                    {orderedLocations.length === 0 ? (
                                        ...
                                        orderedLocations.map((loc) => (
                                            <tr key={loc.id}
                                                ...
                                                <td style={{ ...tdStyle, color: "#3d4758", fontFamily: pageFontFamily, whiteSpace: "nowrap", minWidth: 150 }}>
                                                    {loc.primaryContact || <span style={{ color: "#bbb" }}>-</span>}
                                                </td>
                                                <td style={{ ...tdStyle, color: "#3d4758", fontFamily: pageFontFamily, whiteSpace: "nowrap", minWidth: 120 }}>
                                                    {loc.phone || <span style={{ color: "#bbb" }}>-</span>}
                                                </td>
