...
interface CreditNotePrefs {
    allowOverrideCostPrice: boolean;
...
const QR_CODE_TYPES = [
    { value: 'upi-id', label: 'UPI ID', description: 'Your UPI ID will be displayed as a QR code on credit notes.' },
    { value: 'credit-note-url', label: 'Credit Note URL', description: 'When scanned, credit note URL will be displayed to the customer.' },
    { value: 'custom', label: 'Custom', description: 'When scanned, the custom URL you configure or the information you enter will be displayed.' },
];
    ...
    return (
        <div ref={ref} style={{ position: 'relative', width: '100%' }}>
            <button
                type="button"
                onClick={() => setOpen(o => !o)}
                style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', fontSize: 14, fontWeight: 500, color: '#212529', background: '#fff', border: '1px solid #ced4da', borderRadius: 6, cursor: 'pointer', outline: 'none' }}
            >
                <span>{selected.label}</span>
                <i className={`ti ti-chevron-${open ? 'up' : 'down'}`} style={{ fontSize: 16, color: '#6c757d' }} />
            </button>
            {open && (
                <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: '#fff', border: '1px solid #ced4da', borderRadius: 6, boxShadow: '0 4px 16px rgba(0,0,0,0.12)', zIndex: 1050 }}>
                    <div style={{ padding: '8px 10px', borderBottom: '1px solid #f0f0f0' }}>
                        <div style={{ position: 'relative' }}>
                            ...
                            <input type="text" autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search"
                                style={{ width: '100%', padding: '6px 8px 6px 30px', fontSize: 14, border: '1px solid #e0e0e0', borderRadius: 4, outline: 'none' }} />
                        </div>
                    </div>
                        ...
                        {filtered.map(opt => {
                            ...
                            return (
                                <div key={opt.value} onClick={() => { onChange(opt.value); setOpen(false); setSearch(''); }}
                                    style={{ padding: '12px 14px', cursor: 'pointer', background: isSelected ? '#FFF0EE' : '#fff', borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, transition: 'background 0.15s' }}
                                ...
                                >
                                    ...
                                    {isSelected && <i className="ti ti-check" style={{ color: '#e41f07', fontSize: 16, flexShrink: 0, marginTop: 2 }} />}
                                </div>
                            );
                        })}
                        {filtered.length === 0 && <div style={{ padding: '16px', textAlign: 'center', color: '#adb5bd', fontSize: 14 }}>No results found</div>}
                    </div>
                </div>
            )}
        </div>
...
const CreditSettings: React.FC = () => {
    ...
    const defaultPrefs: CreditNotePrefs = {
        allowOverrideCostPrice: false,
        qrCodeEnabled: false,
        qrCodeType: 'credit-note-url',
        qrCodeDescription: '',
        termsAndConditions: '',
        customerNotes: '',
    };
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            showToast('success', 'Saved', 'Credit note preferences have been updated.');
            setLoading(false);
        }, 600);
    };
                                ...
                                <div className="card-body px-4 py-4">
                                    ...
                                    {activeTab === 'preferences' && (
                                        <form onSubmit={handleSave} style={{ maxWidth: 820 }}>
                                            ...
                                            <div className="py-2">
                                                <h6 className="fw-bold mb-3" style={{ fontSize: 14, color: '#1a1a2e' }}>Cost Price Preference</h6>
                                                <div className="d-flex align-items-start gap-2">
                                                    <input
                                                        className="form-check-input mt-1"
                                                        type="checkbox"
                                                        id="allowOverrideCostPrice"
                                                        style={{ width: 16, height: 16, flexShrink: 0 }}
                                                        checked={prefs.allowOverrideCostPrice}
                                                        onChange={e => setP('allowOverrideCostPrice', e.target.checked)}
                                                    />
                                                    <div>
                                                        <label className="form-check-label text-dark" htmlFor="allowOverrideCostPrice" style={{ fontSize: 14 }}>
                                                            Allow users to override cost prices in credit notes
                                                        </label>
                                                        <p className="text-primary mt-1 mb-0" style={{ fontSize: 14 }}>
                                            ...
                                            <div className="py-2">
                                                <div className="d-flex align-items-center justify-content-between mb-2">
                                                    <h6 className="fw-bold mb-0" style={{ fontSize: 14, color: '#1a1a2e' }}>Credit Note QR Code</h6>
                                                    ...
                                                    <div className="pref-sub-section py-2 mt-2">
                                                        <div className="row mb-4 align-items-start">
                                                            ...
                                                            <div className="col-md-9">
                                                                <QrCodeTypeSelect value={prefs.qrCodeType} onChange={v => setP('qrCodeType', v)} />
                                                            </div>
                                                        </div>
                        ...
                        <div className="modal-body py-0 custom-scrollbar" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                            <div className="px-1">
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="d-flex align-items-center gap-2">
                                        <input type="checkbox" id="add-pii-cn" className="form-check-input me-1 mt-0" checked={addForm.dataPii || false}
                                            onChange={() => setAddForm(p => ({ ...p, dataPii: !p.dataPii }))} style={{ width: 16, height: 16, cursor: 'pointer', float: 'none' }} />
                                        <label htmlFor="add-pii-cn" style={{ cursor: 'pointer', fontSize: 14, marginBottom: 0 }}>PII (Personal Identifiable Information)</label>
                                    </div>
                                    ...
                                    <div className="d-flex align-items-center gap-4">
                                        {(['Yes', 'No'] as const).map(v => (
                                            <div key={v} className="d-flex align-items-center gap-2">
                                                <input type="radio" id={`add-dup-cn-${v}`} name="add_dup_cn" checked={addForm.preventDuplicates === v}
                                                    onChange={() => setAddForm(p => ({ ...p, preventDuplicates: v }))}
                                                    style={{ accentColor: '#e41f07', width: 15, height: 15, cursor: 'pointer', float: 'none' }} />
                                                <label htmlFor={`add-dup-cn-${v}`} style={{ cursor: 'pointer', marginBottom: 0, fontSize: 14 }}>{v}</label>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    <DefaultValueInput dataType={addForm.dataType || 'Text Box (Single Line)'} value={addForm.defaultValue || ''}
                                        onChange={v => setAddForm(p => ({ ...p, defaultValue: v }))} idPrefix="add-cn" hasError={!!addErr.defaultValue} />
                        ...
                        <div className="modal-body py-0 custom-scrollbar" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                            <div className="px-1">
                                ...
                                <div className="mb-3">
                                    <label className="form-label text-dark fw-bold mb-1" style={{ fontSize: 14 }}>Data Privacy</label>
                                    <div className="d-flex align-items-center gap-2">
                                        <input type="checkbox" id="edit-pii-cn" className="form-check-input me-1 mt-0" checked={editForm.dataPii || false}
                                            onChange={() => setEditForm(p => ({ ...p, dataPii: !p.dataPii }))} style={{ width: 16, height: 16, cursor: 'pointer', float: 'none' }} />
                                        <label htmlFor="edit-pii-cn" style={{ cursor: 'pointer', fontSize: 14, marginBottom: 0 }}>PII</label>
                                    </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="d-flex align-items-center gap-4">
                                        {(['Yes', 'No'] as const).map(v => (
                                            <div key={v} className="d-flex align-items-center gap-2">
                                                <input type="radio" id={`edit-dup-cn-${v}`} name="edit_dup_cn" checked={editForm.preventDuplicates === v}
                                                    onChange={() => setEditForm(p => ({ ...p, preventDuplicates: v }))}
                                                    style={{ accentColor: '#e41f07', width: 15, height: 15, cursor: 'pointer', float: 'none' }} />
                                                <label htmlFor={`edit-dup-cn-${v}`} style={{ cursor: 'pointer', marginBottom: 0, fontSize: 14 }}>{v}</label>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="mb-3">
                                    ...
                                    <DefaultValueInput dataType={editForm.dataType || 'Text Box (Single Line)'} value={editForm.defaultValue || ''}
                                        onChange={v => setEditForm(p => ({ ...p, defaultValue: v }))} idPrefix="edit-cn" />
                                </div>
                                <div className="d-flex align-items-center justify-content-between mb-3 border-bottom pb-2">
                                    <label className="form-label mb-0 text-dark fw-bold" style={{ fontSize: 14 }}>Mandatory</label>
                                    <div className="form-check form-switch mb-0">
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="d-flex align-items-center gap-4">
                                        {(['Active', 'Inactive'] as const).map(v => (
                                            <div key={v} className="d-flex align-items-center gap-2">
                                                <input type="radio" id={`edit-st-cn-${v}`} name="edit_status_cn" checked={editForm.status === v}
                                                    onChange={() => setEditForm(p => ({ ...p, status: v }))}
                                                    style={{ accentColor: '#e41f07', width: 15, height: 15, cursor: 'pointer', float: 'none' }} />
                                                <label htmlFor={`edit-st-cn-${v}`} style={{ cursor: 'pointer', marginBottom: 0, fontSize: 14 }}>{v}</label>
                                            </div>
                                        ))}
                                    </div>
                                </div>
